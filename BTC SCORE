...
try:
    funding = requests.get("https://fapi.binance.com/fapi/v1/premiumIndex?symbol=BTCUSDT").json()[0]['lastFundingRate']
...
score_dxy = normalize(-dxy_change, -5, 5)          # DXY caindo = bom
score_yield = normalize(-tnx_change, -0.5, 0.5)    # Yield caindo = bom
score_gold = normalize(-gold_change, -5, 5)        # Ouro caindo = rotação
score_fg = (100 - fg_value)                        # Medo = oportunidade
score_funding = normalize(-funding_pct, -0.1, 0.1)
score_rsi = normalize(rsi, 20, 80)                 # 30-50 ideal

# ETF Flow (você atualiza manualmente ou cola do CoinGlass/Newhedge)
...
with col_b:
    fig = px.gauge(score_final, range=[0, 100], steps=[{"range": [0, 30], "color": "red"}, {"range": [30, 70], "color": "yellow"}, {"range": [70, 100], "color": "green"}], title="Quanto mais alto = melhor momento para comprar")
    st.plotly_chart(fig, use_container_width=True)

st.caption("Atualizado em tempo real • Baseado no fluxo de capital pós-deleveraging (guerra Irã-EUA 2026)")
